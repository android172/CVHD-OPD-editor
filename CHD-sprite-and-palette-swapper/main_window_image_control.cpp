...
void MainWindow::on_list_images_currentItemChanged(QListWidgetItem *current, QListWidgetItem *previous) {
    ...
    for (int i = 0; i < width; ++i) {
        for (int j = 0; j < height; ++j) {
            Color c = _pallete_colors[pixels[i][j]];
...
void MainWindow::on_bt_image_to_clip_clicked() {
    auto current_image = dynamic_cast<ImageLWI*>(ui->list_images->currentItem());
...
void MainWindow::import_images(QStringList path_list) {
    ...
    for (const auto& file_path : path_list) {
        ...
        // Preform initialization if necessery
        if (_image_list_empty) {
            initial_load(image);
        }
...
void MainWindow::dropEvent(QDropEvent *event) {
    ...
    for (int i = 0; i < mime_data->urls().size(); ++i) {
        QString url = mime_data->urls().at(i).toString();
        if (url.startsWith("file:///")) {
            auto splits = url.split('.');
            QString extension = splits[splits.size() - 1];
            if (supported_image_formats.contains(extension)) {
                QString path = url.replace("file:///", "");
